...
inline static int policycheck(List *Acqcores,int cpuid){
CpuNode *temp;
temp=Acqcores->TailCpuNode;
	...
	{
		if(temp->cpuid%H_THREADS==cpuid%H_THREADS)	{
			return true;
		}
	}
	return false;
...
//starts invading cores
 ...
 Root=B;
	...
	cluster=RandomCluster(app->cores);
	...
	for(int i=0;i<app->cores;i++)
	{
		...
		{	while(SWAP(&cluster->parentpool->update_lock,update_lock));
			...
			node=pop(cluster);
			pass=policycheck(Acqcores,node.cpuid);
			if(!pass){

            cluster->TailCpuNode->next=node;
            cluster->TailCpuNode=node;
			i--;
		...
		else
		{
			...
			if(cluster==NULL){
				...
				cluster->parentpool->update_lock=false;
				cluster=RandomCluster(app->cores-i);
				if(cluster==NULL)
				{
					return;
				}
